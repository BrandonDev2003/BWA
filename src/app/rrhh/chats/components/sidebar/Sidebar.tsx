...
export default function SidebarRRHH() {
  ...
  const initial = (me?.nombre?.trim()?.[0] || "R").toUpperCase();
  const title = me?.nombre?.trim() || "RRHH";
  const subtitle = me?.correo?.trim() || "Recursos Humanos";
  const roleLabel = (me?.rol || "rrhh").toUpperCase();

  // widths para empujar contenido
  const wOpen = 240;
  const wClosed = 76;
  ...
  return (
    <>
      ...
      {/* ✅ SIDEBAR FIXED: top+bottom (NO vh) */}
      <motion.aside
      ...
      >
        ...
        {/* ✅ ZONA SCROLL SOLO DENTRO DE LA SIDEBAR */}
